...
class CommandBar(Static):
    def on_mount(self) -> None:
        # Short, Claude-like hint bar above the prompt
        self.update(
            "Type commands like :loc, :interval, :pause, :resume, :digest, :clear, :q"
        )


class CommandInput(Input):
    def on_mount(self) -> None:
        # Claude-style inline hint
        self.placeholder = "weather · :help"
        self.focus()
...
class WeatherApp(App):
    ...
    Layout:
      ┌ header (single status line)
      ├ main log (weather + command echoes)
      ├ summary sidebar (right)
      ├ hint bar
      └ single-line command prompt at bottom
    """
    ...
    async def _set_location(self, location_str: str) -> None:
        ...
        try:
            # Offload blocking geocode + NOAA points fetch to worker threads
            ctx = await self.run_in_thread(
                build_location_context,
                location_str,
                "weather-tui/1.0",
            )
            ...
            points_json = await self.run_in_thread(
                get_json,
                points_url,
                HEADERS,
            )
            ...
            view.push_panel(
                Panel(
                    f"Location set to [bold]{ctx.name}[/bold].",
                    border_style="green",
                )
            )
    ...
    async def _tick_weather(self) -> None:
        ...
        try:
            period = await self.run_in_thread(
                fetch_forecast_period,
                self._forecast_url,
            )
        except Exception as e:
        ...
        try:
            aqi = await self.run_in_thread(fetch_aqi, ctx.lat, ctx.lon)
        except Exception as e:
            aqi = "N/A"
            view.push_panel(Panel(f"[red]AQI fetch failed:[/red] {e}", border_style="red"))

        try:
            uv = await self.run_in_thread(fetch_uv, ctx.lat, ctx.lon)
        except Exception as e:
            uv = "N/A"
    ...
    async def _handle_command(self, raw: str) -> None:
        ...
        view.push_panel(
            Panel(
                f"[bold magenta]weather ❯[/bold magenta] {raw}",
        ...
        if raw == ":digest":
            try:
                ...
                digest = await self.run_in_thread(get_forecast_digest, ctx.lat, ctx.lon)
